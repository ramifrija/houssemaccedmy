...
-- 3. Mettre à jour la fonction get_conversation_messages
...
-- =============================================================================
-- MIGRATION 036 — SONDAGES: VISIBILITÉ DES VOTANTS ET ANNULATION
-- =============================================================================

-- 1. Fonction pour voter sur un sondage
CREATE OR REPLACE FUNCTION public.vote_on_poll(
  p_message_id UUID,
  p_option_index INTEGER
)
...
-- 2. Fonction pour obtenir les résultats d'un sondage avec les noms des votants
DROP FUNCTION IF EXISTS public.get_poll_votes(UUID);
...
BEGIN
  ...
  SELECT 
    ...
    json_agg(json_build_object(
      'id', p.user_id,
      'name', COALESCE(NULLIF(TRIM(p.first_name || ' ' || p.last_name), ''), p.email, 'Utilisateur')
    )) AS voters
  FROM public.poll_votes pv
